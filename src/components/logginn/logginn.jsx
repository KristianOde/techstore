  ...
  async function onSubmit(e) {
    ...
    return (
      ...
      <form className="form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <input className="forminput" name="brukernavn" id="brukernavnId" ref={register({ required: true, minLength: 5 })}/>
        ...
        <input className="forminput" name="passord" id="passordID" ref={register({ required: true })}/>

        {errors.passord && errors.passord.type === "required" && (
        <h1 className="formerror">Required</h1>)}

        <input disabled={loading} type="submit" className="formknapp"/>
        <p className="message">Har ikke bruker? <Link to="/registrer">Lag en konto</Link></p>
      </form>
      </div>
    );
}
